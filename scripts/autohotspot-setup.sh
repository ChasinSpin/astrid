...
{
	echo
	echo "List of networks in wpa_supplicant:"
	echo
	NETWORKS=`wpa_cli list_networks | sed '1,/flags/d' | cut -f1,2`
	ct=`echo "$NETWORKS" | wc -l`
	
	echo "$NETWORKS"
	echo "$ct       CANCEL (do not remove any networks)"
	echo

	echo -n "Enter the network number you wish to delete (and hit enter or return)> "
        read wf
        if [[ $wf =~ ^[0-9]+$ ]]; then
                if [ $wf -ge 0 ] && [ $wf -lt $ct ]; then
			wpa_cli remove_network "$wf"
			wpa_cli save_config
			echo "Network: $wf REMOVED!"
			echo "Restart now to update networks"
                else
			echo "Cancelling..."
                fi
        else
...
go()
{
	...
	if [ "$opt" = "REM" ] ;then
		...
		echo "Local DNS Stopped"
		echo "Ethernet should now be able to connect to the internet (restart to restore to regular operation)"
	else
		hostapd_config
		dnsmasq_config
		interface
