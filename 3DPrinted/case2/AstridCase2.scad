// FDM Printing New Version:
//
//  Bambu Labs:
//      Print partNum 6 (Plate)
//      
//      Filament Color:  Main: ASA Green (closer to Turquoise), Highlight Only: ASA Black (or ABS Black):  DO NOT USE BLACK FOR THE CASE
...
// Post Processing:
//      - Remove Camera Holder support
//      - Remove Mouse Ears with hobby knife
//      - Wash adhesive off parts
//      - Dry parts
//      - Insert 8 heat inserts in bottom case, with drill press / soldering iron/ supplied heat tool, just below surface
//        (Uses 8 x   M2.5x3.5x4 Hanglife Inserts HLTI-M2.5-SET01)
//      - Use chisel hobby knife to remove proud ASA where insert was put in
...
// Case uses black for highlights, but not for the majority of the case
...
partNum                     = 6;            // 0 = All, 1 = Camera Holder, 2 = Camera Posts, 3 = Bottom Case, 4 = Top Case, 5 = Cable Clamp, 6 = Plate (works), 7 = Gps and Camera washers, 8 = Fan Cover, 9 = Gps and Camera Washers x 56, 10 = Logo
...
fanPostDiameter             = 2.6;
fanCoverPostHoleDiameter    = fanPostDiameter + 0.4;
...
gpsExternalCableHoleDiameter= 6.5 + 0.2;
...
flexCollar2FilledPartHeight = 2.0;
...
module camCableBlanking()
{       
    translate( [0, 0, camCableBlankingFlangeHeight/2] )
        cube( [caseCamCableHoleDimensions[0] + camCableBlankingFlangeExtra, caseCamCableHoleDimensions[1] + camCableBlankingFlangeExtra, camCableBlankingFlangeHeight], center = true );

    translate( [0, 0, caseBottomTopThickness/2 + camCableBlankingFlangeHeight] )
        cube( [caseCamCableHoleDimensions[0]-camCableBlankingClearance, caseCamCableHoleDimensions[1]-camCableBlankingClearance, caseBottomTopThickness], center = true );
}
...
module logo3Line(offsetY, thickness)
{
    translate( [0, offsetY, 0] )
        rotate( [0, 0, -30] )
            translate( [0, 0, (logoDepth + manifoldCorrection)/2] )
    ...
    difference()
    {
        ...
        translate( [0, 0, caseSplitHeight - caseHeight] )
            translate( [0, 0, caseHeight/2 - manifoldCorrection] )
                cube( [caseOuterBorder[0], caseOuterBorder[1] * 1.2, caseHeight + manifoldCorrection * 2], center = true);
        translate( [-piBoardDimensions[0]/2, -piBoardDimensions[1]/2, caseBottomTopThickness + caseBottomPiClearance] )
    raspberryPi4BModel();
    ...
    difference()
    {
            ...
            ])
        {
            translate( [pos[0], pos[1], 0] )
            {
                ...
                translate( [0, 0, (caseSplitHeight - insertHoleDepth)] )
        ...
        difference()
        {
            ...
            for ( pos = [
                ...
                translate( [pos[0], pos[1]] )
                    cylinder(d=insertOuterDiameter, h=insertHoleDepth);
        }
        ...
        for ( pos = [
            ...
            translate( [pos[0], pos[1]] )
            {
                translate( [0, 0, -manifoldCorrection] )
                   cylinder(d=insertHoleDiameter, h=insertHoleDepth + manifoldCorrection * 2);
            }
...
{
    mouseEarX = (caseOuterInset[0]+ caseOuterInsetInset)/2;
    mouseEarY = (caseOuterInset[1] + caseOuterInsetInset)/2;
    ...
    for (pos = [
        ...
        {
            mouseEar();
            translate( [0, 0, height - mouseEarThickness] )
                mouseEar();
        }
...
module flexCollar2Filled()
{
    flexCollar2();
    lozenge(flexOuterClampOuterLength + flangeAdditional, flexOuterClampOuterWidth + flangeAdditional, flexCollar2FilledPartHeight);
}
