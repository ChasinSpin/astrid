...
"""
Here is the code for computing the predicted event center time and the chord distance (fundamental plane) from a given location.

* timeAndChordDistanceForLocationFP() - added this new routine which computes the min approach and chord distance in the fundamental plane.  
* C_Mu - added this new constant
* dateTimeAtClosest - I added code to include the fractional seconds. 

You will likely find a few random additional changes related to debugging.  I haven't done a lot of testing yet.  Just a few checks against my C# code.

Apparent position of the target star:
Use the position from the occelmnt file.  Occult uses this position when computing the bessellian elements.  You should use the same star position when computing the path locations.  I'm sure it is possible to compute the same position using AstroPy.  But the standard frames won't yield exactly the same apparent position computed by Occult.

I recommend you change your approach to coordinate transformations in astcoord.py.  You should avoid FK5.  Instead, just use the 'TETE' reference frame for JNOW.  'TETE' is a version of the apparent reference frame (looks to be slightly different from the frame used in Occult).   From a practical standpoint, TETE is the JNOW reference frame.

The Ra,Dec coordinates (first pair) in the occelmnt file are astrometric: ICRS position of the star including proper motion to the event date.  To obtain the apparent/JNOW position at the event date:
	def apparentFrom24Deg(cls, ra: float, dec: float, obsdatetime = None):
		skyCoord = SkyCoord(ra,dec,frame='icrs',unit=(u.hourangle,u.deg))
		skyCoord = skyCoord.transform_to(TETE(obstime = obsdatetime))
		return cls(skyCoord)
...
class PathComputation:
	...
	def __init__(self, occelmnt: dict, calcApparentStar = False):
		"""
		...
		# determine apparent position of star
		if self.calcApparentStar:
			(appRA, appDE)	= self.starCoord.raDec360Deg('icrs', jnow = True, obsdatetime=self.dateTimeAtClosest)
			self.apparentStarCoord = AstCoord.from360Deg(appRA,appDE,'icrs')
			print('Star Apparent (calc):', self.apparentStarCoord.raDecHMSStr('icrs'))
		else:
			self.apparentStarCoord	= AstCoord.from24Deg(float(star[9]), float(star[10]), 'icrs')
			print('Star Apparent (occelmnt):', self.apparentStarCoord.raDecHMSStr('icrs'))
	...
	def timeAndChordDistanceForLocationFP(self, lat: float, long: float, elev: float, toleranceKM: float, maxRange: float) -> (float, float):
		"""
		Inputs:
			lat,long,height = observer's lat, E long and elevation above ellipsoid
...
# 2023-08-16 Petrina event : predicted min time, distance for 49.9925, -11.751944,  should be 08:37:42 UTC and 11.41 km
#
pc = PathComputation(occelmnt_test)
pc.timeAndChordDistanceForLocation(49.9925, -111.751944)

minApproach = pc.timeAndChordDistanceForLocationFP(49.9925,-111.751944,1.125,0.1,2.0)
#minApproach = pc.timeAndChordDistanceForLocationFP(0.0,0.0,1.125,0.1,2.0)
if (minApproach is None):
	print("** Star not visible from this location.")
else:
	minHrs = minApproach[0]
	minChord = minApproach[1]
	dteMin = pc.dateTimeAtClosest + timedelta(hours=minHrs)
	print( "Min approach at " + str(dteMin) +  " at " + str(minChord) + " km")
